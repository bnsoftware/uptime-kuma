...
# curl -o kuma_install.sh https://raw.githubusercontent.com/bnsoftware/uptime-kuma/master/install.sh && sudo bash kuma_install.sh
"echo" "-e" "====================="
...
if [ "$type" == "local" ]; then
  ...
  mkdir -p $installPath
  cd $installPath
  git clone https://github.com/bnsoftware/uptime-kuma.git .
  npm run setup
  ...
  docker run -d --restart=always -p $port:3001 -v $volume:/app/data --name uptime-kuma bnsoftware/uptime-kuma:1
fi
